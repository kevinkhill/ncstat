...
const isIdle = (0, _fp.eq)(_NcService.NcMachineStates.IDLE);
const isToolpathing = (0, _fp.eq)(
  _NcService.NcMachineStates.TOOLPATHING
);
const isInCannedCycle = (0, _fp.eq)(
  _NcService.NcMachineStates.IN_CANNED_CYCLE
);
...
class NcParser extends _NcEventEmitter.NcEventEmitter {
  ...
  constructor({ debug }) {
    ...
    this.blocks = [];
    this.nc = void 0;
    this.state = _NcService.NcMachineStates.IDLE;
    this.lexer = void 0;
    this.tokens = [];
  ...
  parse(source) {
    ...
    this.program.forEach(block => {
      ...
      if (block.isStartOfCannedCycle && isToolpathing(this.state)) {
        this.nc.send(_NcService.NcMachineEvents.START_CANNED_CYCLE);
      ...
      if (isInCannedCycle(this.state)) {
        if (block.G.includes(80)) {
          this.nc.send(_NcService.NcMachineEvents.END_CANNED_CYCLE);
        }
      ...
      if (block.toString().startsWith("N")) {
        if (isToolpathing(this.state)) {
          this.nc.send(_NcService.NcMachineEvents.END_TOOLPATH);
        ...
        if (isIdle(this.state)) {
          ...
          this.nc.send(_NcService.NcMachineEvents.START_TOOLPATH);
        }
      }

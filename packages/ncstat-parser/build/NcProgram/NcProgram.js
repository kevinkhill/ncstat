...
const HEADER_START_LINE = 2;
exports.HEADER_START_LINE = HEADER_START_LINE;
...
class NcProgram {
  constructor() {
    ...
    this.defaults = {
      headerSeparator: " - ",
      // Taken from the default Mastercam post
  ...
  get regionSpans() {
    ...
    [...this.blankLines, this.blockCount].forEach(lineNumber => {
      var _regionSpans$to, _regionSpans;

      regionSpans.push({
        from:
          ((_regionSpans$to =
            (_regionSpans = regionSpans[regionSpans.length - 1]) ===
              null || _regionSpans === void 0
              ? void 0
              : _regionSpans.to) !== null && _regionSpans$to !== void 0
            ? _regionSpans$to
            : 0) + 2,
        to: lineNumber - 1
      });
  ...
  getAxisValues(axis) {
    const values = (0, _fp.uniq)(
      (0, _fp.map)((0, _fp.get)(axis), this.blocks)
    );
    return (0, _fp.reject)(isNaN, values);
  }
  ...
  getAxisLimits(axis) {
    ...
    return {
      min:
        (_min = (0, _fp.min)(values)) !== null && _min !== void 0
          ? _min
          : NaN,
      max:
        (_max = (0, _fp.max)(values)) !== null && _max !== void 0
          ? _max
          : NaN
    };
  }
  ...
  getLines(from, to) {
    const blocks = this.blocks.slice(from, to + 1); // console.log(blocks);
  ...
  getRegion(from, to) {
    return _NcRegion.NcRegion.create(this.getLines(from, to));
  }
  /**
  ...
  getStats() {
    return {
      ...
      toolpaths: {
        count: this.toolpathCount
      },
      toolchanges: {
        count: this.toolchangeCount
      }
    };
  }
  ...
  toStringWithLineNumbers() {
    return this.blocks
      .map((value, index) => `N${(0, _lib.zeroPad)(index)} ${value}`)
      .join("\n");
  }
  /**
  ...
  collectCommentsFrom(start) {
    ...
    for (const block of this.blocks.slice(start)) {
      ...
      if (block.comment)
        comments.push(
          block === null || block === void 0 ? void 0 : block.comment
        );
    }

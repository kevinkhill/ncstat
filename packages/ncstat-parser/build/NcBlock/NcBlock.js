...
class NcBlock {
  ...
  $has(prefix) {
    return (0, _lexer.filterByPrefix)(prefix, this.tokens).length > 0;
  }

  $value(prefix) {
    if (this.$has(prefix)) {
      const token = (0, _lexer.filterByPrefix)(prefix, this.tokens)[0];
  ...
  get G() {
    return (0, _fp.map)(
      _lexer.addressValue,
      (0, _lexer.filterByPrefix)("G", this.tokens)
    );
  }
